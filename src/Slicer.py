...
IMAGE_NAME = 'DOP25_LV95_1091-14_2013_1_13.tif'
...
BASE_OVERLAP_FACTOR = 0.20
MAP_OFFSET = (0, 0)

X_SIZE, Y_SIZE = ORTHOPHOTO.size
BASE_OVERLAP = BOX_SIZE * BASE_OVERLAP_FACTOR
OVERLAP_OFFSET_X = math.floor(BOX_SIZE - X_SIZE / math.floor(X_SIZE / (BOX_SIZE - BASE_OVERLAP) + 1))
OVERLAP_OFFSET_Y = math.floor(BOX_SIZE - Y_SIZE / math.floor(Y_SIZE / (BOX_SIZE - BASE_OVERLAP) + 1))

print('X overlap: ' + str(OVERLAP_OFFSET_X) + " ({0:.00%})".format(OVERLAP_OFFSET_X / BOX_SIZE))
print('Y overlap: ' + str(OVERLAP_OFFSET_Y) + " ({0:.00%})".format(OVERLAP_OFFSET_Y / BOX_SIZE))

TILES = []
for x in range(0, X_SIZE, BOX_SIZE - OVERLAP_OFFSET_X):
    for y in range(0, Y_SIZE, BOX_SIZE - OVERLAP_OFFSET_Y):
        tile = {}
        tile['tile'] = ORTHOPHOTO.crop((x, y, x + BOX_SIZE, y + BOX_SIZE))
        tile['a'] = (x, y)
        tile['b'] = (x + BOX_SIZE, y + BOX_SIZE)
        TILES.append(tile)
...
for i, tile in enumerate(TILES):
    tileName = "{}/{:0>3d},{:0>3d}.png".format(
        OUT_DIRECTORY, tile['a'][1], tile['a'][0])
    tile['tile'].save(tileName, "PNG")
